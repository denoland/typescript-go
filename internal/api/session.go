...
type Session struct {
	...
	SetForkContextInfo func(forkContextInfo ast.DenoForkContextInfo)
...
func NewSession(projectSession *project.Session, options *SessionOptions) *Session {
	...
	if options != nil {
		...
		s.SetForkContextInfo = options.SetForkContextInfo
	}
	return s
}
...
func (s *Session) HandleRequest(ctx context.Context, method string, params json.Value) (any, error) {
	...
	case string(MethodInitialize):
		return s.handleInitialize(ctx, parsed.(*InitializeParams))
	...
	case string(MethodGetDiagnostics):
		return s.handleGetDiagnostics(ctx, parsed.(*GetDiagnosticsParams))
...
func (s *Session) handleInitialize(ctx context.Context, params *InitializeParams) (*InitializeResponse, error) {
	s.SetForkContextInfo(ast.DenoForkContextInfo{
		TypesNodeIgnorableNames: collections.NewSetFromItems(params.ForkContextInfo.TypesNodeIgnorableNames...),
		NodeOnlyGlobalNames:     collections.NewSetFromItems(params.ForkContextInfo.NodeOnlyGlobalNames...),
	})
...
func (s *Session) handleGetMembersOfSymbol(ctx context.Context, params *GetMembersOfSymbolParams) ([]*SymbolResponse, error) {
	...
	results := make([]*SymbolResponse, 0, symbol.Members.Len())
	for _, member := range symbol.Members.Iter() {
...
func (s *Session) handleGetExportsOfSymbol(ctx context.Context, params *GetExportsOfSymbolParams) ([]*SymbolResponse, error) {
	...
	results := make([]*SymbolResponse, 0, symbol.Exports.Len())
	for _, exp := range symbol.Exports.Iter() {
...
func (s *Session) handleGetDiagnostics(ctx context.Context, params *GetDiagnosticsParams) ([]*ls.Diagnostic, error) {
	sd, err := s.getSnapshotData(params.Snapshot)
	if err != nil {
		return nil, err
	}
	projectPath := parseProjectHandle(params.Project)
	project := sd.snapshot.ProjectCollection.GetProjectByPath(projectPath)
	if project == nil {
		return nil, errors.New("project not found")
	}
	languageService := ls.NewLanguageService(projectPath, project.GetProgram(), sd.snapshot, "")
	diagnostics := languageService.GetDiagnostics(ctx, params.FileNames)
