...
const (
	...
	callbackResolveJsxImportSource          = "resolveJsxImportSource"
	callbackResolveModuleName               = "resolveModuleName"
	callbackResolveTypeReferenceDirective   = "resolveTypeReferenceDirective"
	callbackGetPackageJsonScopeIfApplicable = "getPackageJsonScopeIfApplicable"
	callbackGetPackageScopeForPath          = "getPackageScopeForPath"
	callbackGetImpliedNodeFormatForFile     = "getImpliedNodeFormatForFile"
	callbackIsNodeSourceFile                = "isNodeSourceFile"
)
...
func (fs *callbackFS) ReadFile(path string) (contents string, ok bool) {
	if fs.isEnabled(callbackReadFile) && !strings.HasPrefix(path, "bundled://") {
